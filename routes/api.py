...
async def get_analysis_status(repo_id: str, db: AsyncSession = Depends(get_db)):
    """
...
async def ask_question(request: AskQuestionRequest, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        result = await analysis_service.answer_question(
            request.repo_id, request.question, db
        )
...
async def compare_repositories(
    ...
    try:
        result = await comparative_service.compare_repositories(
            compare_request.repo_ids, db, compare_request.comparison_type
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_code_quality(repo_id: str, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        tech_stack = [
            {"name": t.name, "category": t.category} for t in tech_result.scalars()
        ]
