...
class CodeQualityAnalyzer:
    ...
    async def analyze(
        self, files: List[Dict], readme: str = None, tech_stack: List[Dict] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_documentation_score(self, readme: str, files: List[Dict]) -> float:
        ...
        doc_files = [
            f for f in files if f.get("path", "").endswith((".py", ".js", ".java"))
        ]
        if doc_files:
    ...
    def _analyze_organization(self, files: List[Dict]) -> float:
        ...
        has_config = any(
            path.endswith((".json", ".yml", ".yaml", ".toml")) for path in file_paths
        )
    ...
    def _analyze_dependencies(self, files: List[Dict], tech_stack: List[Dict]) -> float:
        ...
        has_ci = any(
            ".github/workflows" in path or ".gitlab-ci" in path for path in file_paths
        )
        if has_ci:
            score += 1.0
