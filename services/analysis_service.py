...
class AnalysisServiceFinal:
    ...
    async def start_analysis(self, repo_url: str, db: AsyncSession) -> Dict:
        ...
        return {"repo_id": repo_id, "session_id": session_id, "status": "processing"}
    ...
    async def execute_analysis(self, repo_id: str, db: AsyncSession) -> None:
        ...
        try:
            ...
            important_files = self.file_filter.filter_important_files(
                tree, max_files=30
            )
            ...
            # Fetch issues
            open_issues = await self.github.get_issues(
                owner, repo_name, state="open", max_issues=30
            )
            closed_issues = await self.github.get_issues(
                owner, repo_name, state="closed", max_issues=20
            )

            print(
                f"✓ GitHub data fetched: {len(important_files)} files, {len(open_issues)} open issues"
            )
            ...
            context = {
                ...
                "config_files": [
                    f for f in important_files if f["role"] == "configuration"
                ],
                "source_files": [
                    f
            ...
            print(
                f"✓ Received structured analysis (confidence: {analysis.confidence_score})"
            )
            ...
            await db.execute(select(TechStack).where(TechStack.repo_id == repo_id))
            # Delete existing
            result = await db.execute(
            ...
            result = await db.execute(
                select(ArchitectureComponent).where(
                    ArchitectureComponent.repo_id == repo_id
                )
            )
            ...
            # 4. Key Files
            result = await db.execute(select(KeyFile).where(KeyFile.repo_id == repo_id))
            ...
            for area in analysis.contribution_areas:
                contrib_area = ContributionArea(
                    id=str(uuid.uuid4()), repo_id=repo_id, area=area
                )
                db.add(contrib_area)
            ...
            for risky in analysis.risky_areas:
                risky_area = RiskyArea(
                    id=str(uuid.uuid4()), repo_id=repo_id, area=risky
                )
                db.add(risky_area)
            ...
            for issue in analysis.known_issues:
                known_issue = KnownIssue(
                    id=str(uuid.uuid4()), repo_id=repo_id, issue=issue
                )
                db.add(known_issue)

            # 9. Raw Response (for debugging)
            result = await db.execute(
                select(RawAnalysisResponse).where(
                    RawAnalysisResponse.repo_id == repo_id
                )
            )
    ...
    async def get_analysis(self, repo_id: str, db: AsyncSession) -> Dict:
        ...
        if status_result["status"] != "completed":
            raise ValueError(
                f"Analysis not completed (status: {status_result['status']})"
            )
        ...
        components = [
            {
                "name": c.name,
                "purpose": c.purpose,
                "files": json.loads(c.key_files) if c.key_files else [],
            }
        ...
        return {
            ...
            "analyzed_at": summary.created_at.isoformat()
    ...
    async def answer_question(
        self, repo_id: str, question: str, db: AsyncSession
    ) -> Dict:
        """
        ...
        if raw_response:
            ...
            analysis_obj = RepositoryAnalysis(
                ...
                components=[
                    ComponentItem(**c) for c in analysis_data.get("components", [])
                ],
                data_flow=analysis_data["data_flow"],
                key_files=[
                    FileInsight(**f) for f in analysis_data.get("key_files", [])
                ],
